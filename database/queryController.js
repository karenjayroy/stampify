...
const uri =
  "postgres://sqbtvtnq:C8VT4uFIv9Ynt18DhfhmJb7npcHet97b@isilo.db.elephantsql.com:5432/sqbtvtnq";
...
const addUser = (req, res, next) => {
  client.query(
    'INSERT INTO users ("user_name", "user_password", "phone_number") VALUES ($1, $2, $3) RETURNING *;',
    [req.body.name, req.body.password, req.body.phone_number],
    (err, result) => {
      if (err) {
        console.log(err);
...
const stampCount = (req, res, next) => {
  client.query(
    "select stamp_count, store_name from user_store inner join stores on user_store.store_id = stores.store_id where user_id = $1;",
...
const storeLogin = (req, res, next) => {
  ...
  client.query(
    `SELECT * FROM stores WHERE store_name=$1 and store_password=$2;`,
    [name, password],
    (err, result) => {
      if (err) {
...
const addStamp = (req, res, next) => {
  ...
  client.query(
    "update user_store set stamp_count = (Select stamp_count from user_store inner join users on user_store.user_id = users.user_id where phone_number=$1 and user_store.store_id = $2) + 1 where user_store.store_id = $2 and user_id = (select user_id from users where phone_number = $1)",
    [number, id],
    (err, result) => {
      if (err) {
        return res.status(400).send("Failed login!");
